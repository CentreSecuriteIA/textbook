...
(() => {
      ...
      (function () {
        ...
        module.exports = {
          ...
          score: function (string, query, options) {
            ...
            if (
              !(
                (string != null ? string.length : void 0) && (query != null ? query.length : void 0)
              )
            ) {
              return 0;
            }
          ...
          match: function (string, query, options) {
            ...
            if (string === query) {
              return function () {
                _results = [];
                for (
                  var _i2 = 0, _ref2 = string.length;
                  0 <= _ref2 ? _i2 < _ref2 : _i2 > _ref2;
                  0 <= _ref2 ? _i2++ : _i2--
                ) {
                  _results.push(_i2);
                }
  ...
  function getActiveElement() {
    ...
    return (_d =
      (_c =
        (_b = (_a = document.activeElement) == null ? void 0 : _a.shadowRoot) == null
          ? void 0
  ...
  // src/overrides/assets/javascripts/components/parallax/index.ts
  function watchParallax(el) {
    return merge(
      ...getElements(':scope [hidden]', el).map((child) =>
        watchElementVisibility(child).pipe(
  ...
  function mountParallax(el) {
    return defer(() => {
      ...
      combineLatest([watchElementContentOffset(el), watchElementOffset(group)]).subscribe(
        ([{ y }, offset]) => {
          const header = getElement('header');
          header.classList.toggle('md-header--shadow', y > offset.y);
        }
      );
      return watchParallax(el).pipe(
      ...
      map(() => {
        ...
        const opacity =
          scrollPosition > threshold
            ? Math.max(0, 1 - (scrollPosition - threshold) / threshold)
            : 1;
        console.log({ opacity, transform: `translateY(${-20 * (1 - opacity)}px)` });
        return { opacity, transform: `translateY(${-20 * (1 - opacity)}px)` };
      })
    );
  }
  ...
  function mountHero(el) {
    return defer(() => {
      const push$ = new Subject();
      push$.subscribe(({ opacity, transform }) => {
        el.style.opacity = opacity;
        el.style.transform = transform;
      });
      return watchHeroVisibility(el).pipe(
        tap((state) => push$.next(state)),
        finalize(() => push$.complete()),
        map((state) => __spreadValues({ ref: el }, state))
      );
  ...
  var component$ = document$.pipe(
    switchMap(() =>
      merge(
        ...getComponentElements('parallax').map((el) => mountParallax(el)),
        ...getComponentElements('hero').map((el) => mountHero(el))
      )
    )
  );
  component$.subscribe();
